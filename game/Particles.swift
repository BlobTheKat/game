...
let particles: [(Object) -> Particle] = [
    ...
    { (planet) in
        ...
        return Particle[State(color: (r: 255, g: 255, b: 255), size: CGSize(width: 20, height: 20), zRot: 4, position: CGPoint(x: planet.position.x + dir.dx * 1.3, y: planet.position.y + dir.dy * 1.3), alpha: 0), State(color: (r: 0, g: 0, b: 0), size: CGSize(width: 10, height: 10), zRot: 0, position: CGPoint(x: planet.position.x + dir.dx, y: planet.position.y + dir.dy), alpha: 1, delay: 2)]
